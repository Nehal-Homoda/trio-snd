...
const { submitMakeOrder, orderValidate, saveOrder } = useMakeOrderStore();
const {
  orderFormData,
  orderOptions,
...
<template>
  <div>
    ...
    " class="d-flex align-start align-sm-end justify-space-between flex-column flex-sm-row ga-5">
      ...
      <v-btn color="primary" size="small" prepend-icon="mdi-clipboard-check-outline"
        class="font-size-small font-weight-bold text-white" @click="submitHandler('save')" :disabled="submitLoading"
        :loading="submitLoading && loadingType === 'save'">
         {{ $t('save_order') }}
      </v-btn>
    </div>
    ...
    <v-form ref="createOrderForm" @submit.prevent="submit" class="mt-10">
      <v-row>
        <template v-for="(orderOption, index) in orderOptions?.data">
          <v-col cols="12" sm="6">
            <component :is="dynamicTextFieldComponent(orderOption)" :label="orderOption.name"
              :placeholder="orderOption.placeholder" :items="getItemValues(orderOption)"
              :required="orderOption.is_required" v-model="orderFormData[index].value" :isMultiple="orderOption.type === 'multi_image' ? true : undefined
                " :orderOptionValue="orderOption"></component>
          </v-col>
        </template>
      </v-row>
              ...
              <v-card rounded="lg" :title="$t('target_price')">
                <v-card-text>
                  <v-text-field type="number" v-model="targetPrice" :rules="[(v) => !!v || 'Required!']"
                    variant="solo-filled" :placeholder="$t('enter_your_target_price')">
                  ...
                  <v-btn text="Send" height="45" block @click="submitHandler('targetPrice')" :disabled="!!!targetPrice" variant="elevated" rounded="lg" class="text-white mt-3" color="primary"
