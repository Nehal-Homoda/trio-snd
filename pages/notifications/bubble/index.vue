...
const { notifications, bubbleNotification, loading } = storeToRefs(notificationsStore);
...
<template>
    <v-sheet color="transparent" width="100%" min-height="100vh" class="py-16">
        <v-container>
            <p class="font-weight-black mt-16 mb-6 font-size-large ">{{ $t('bubble_notification') }}</p>

            <UserOrdersCardsNotificationCard v-if="notifications" v-for="(item, index) in bubbleNotification"
                :notification="item" @mark-read="markAsRead(item.id, index)" />

            <div v-if="notifications && !!!bubbleNotification.length && !loading"
